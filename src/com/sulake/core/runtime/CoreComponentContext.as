package com.sulake.core.runtime
{
   ...
   public final class CoreComponentContext extends ComponentContext implements ICore
   {
      ...
      private static const const_106:uint = 3;
      
      private static const const_1514:String = "asset-libraries";
      
      private static const const_1511:String = "library";
      
      private static const const_1517:String = "service-libraries";
      
      private static const const_1515:String = "library";
      
      private static const const_1518:String = "component-libraries";
      
      private static const const_1516:String = "library";
      
      private static const const_1510:String = "error_data";
      
      private static const const_1512:String = "error_cat";
      
      private static const const_1513:String = "error_desc";
       
      
      private var var_380:LibraryLoaderQueue;
      
      private var var_804:IEventDispatcher;
      
      private var var_534:uint;
      
      private var var_381:Function;
      ...
      public function CoreComponentContext(param1:DisplayObjectContainer, param2:uint)
      {
         ...
         var_414 = param1;
         var _loc3_:int = 0;
         while(_loc3_ < CoreComponentContext.const_106)
         {
            this.var_270.push(new Array());
            this.var_651.push(0);
            _loc3_++;
         }
         ...
         switch(param2 & 0)
         {
            case Core.const_1257:
               ...
               this.var_381 = this.simpleFrameUpdateHandler;
               break;
            case Core.const_959:
               debug("Core; using complex frame update handler");
               this.var_381 = this.complexFrameUpdateHandler;
               break;
            case Core.const_766:
               debug("Core; using profiler frame update handler");
               this.var_381 = this.profilerFrameUpdateHandler;
               this.var_211 = new Profiler(this);
               attachComponent(this.var_211,[new IIDProfiler()]);
               var_414.addChild(new ProfilerViewer(this.var_211));
               break;
            case Core.const_621:
               debug("Core; using experimental frame update handler");
               this.var_381 = this.experimentalFrameUpdateHandler;
               break;
            case Core.CORE_SETUP_DEBUG:
               debug("Core; using debug frame update handler");
               this.var_381 = this.debugFrameUpdateHandler;
         }
      }
      
      public function getNumberOfFilesPending() : uint
      {
         return this.var_380.length;
      }
      
      public function getNumberOfFilesLoaded() : uint
      {
         return this.var_534 - this.getNumberOfFilesPending();
      }
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            ...
            {
               i = 0;
               while(i < CoreComponentContext.const_106)
               {
                  receivers = this.var_270[i] as Array;
      ...
      override public function error(param1:String, param2:Boolean, param3:int = -1, param4:Error = null) : void
      {
         if(param4)
         {
            ErrorReportStorage.setParameter(const_1510,String(param4.getStackTrace()));
         }
         ErrorReportStorage.setParameter(const_1512,String(param3));
         ErrorReportStorage.setParameter(const_1513,param1);
      ...
      public function readConfigDocument(param1:XML, param2:IEventDispatcher = null) : void
      {
         ...
         if(this.var_380 == null)
         {
            this.var_380 = new LibraryLoaderQueue(_debug);
         }
         try
         {
            node = config.child(const_1514)[0];
            if(node != null)
            {
               list = node.child(const_1511);
         ...
         {
            error("Failed to parse asset libraries from config xml!",true,Core.const_228,e);
         }
         try
         {
            node = config.child(const_1517)[0];
            if(node != null)
            {
               list = node.child(const_1515);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse interfaces from config xml!",true,Core.const_228,e);
         }
         try
         {
            node = config.child(const_1518)[0];
            if(node != null)
            {
               list = node.child(const_1516);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse components from config xml!",true,Core.const_228,e);
         }
         if(!disposed)
         {
      ...
      private function errorInLoadingProcess(param1:LibraryLoaderEvent = null) : void
      {
         ...
         this.error("Failed to download library \"" + _loc2_.url + "\" HTTP status " + param1.status + ": " + _loc2_.getLastErrorMessage(),true,Core.const_1324);
         if(!disposed)
         {
         ...
         if(param1 != null)
         {
            if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE || param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR)
            {
               ...
               if(!disposed)
               {
                  if(this.var_804)
                  {
                     this.var_804.dispatchEvent(new LibraryProgressEvent(_loc2_.url,this.var_534 - this.var_380.length,this.var_534,_loc2_.elapsedTime));
                  }
               }
            }
         }
      ...
      override public function registerUpdateReceiver(param1:IUpdateReceiver, param2:uint) : void
      {
         ...
         param2 = param2 >= CoreComponentContext.const_106 ? 4294967295 : uint(param2);
         var _loc3_:int = !!this.var_211 ? 0 : this.var_1527 & 0;
         if(_loc3_ == Core.const_621)
         {
            this.var_270[param2].push(new UpdateDelegate(param1,this,param2));
         }
         else
         {
            this.var_270[param2].push(param1);
         }
      }
      ...
      override public function removeUpdateReceiver(param1:IUpdateReceiver) : void
      {
         ...
         var _loc3_:* = null;
         var _loc6_:* = null;
         var _loc4_:int = !!this.var_211 ? 0 : this.var_1527 & 0;
         var _loc5_:int = 0;
         while(_loc5_ < CoreComponentContext.const_106)
         {
            _loc3_ = this.var_270[_loc5_] as Array;
            if(_loc4_ == Core.const_621)
            {
      ...
      private function onEnterFrame(param1:Event) : void
      {
         ...
         this.var_381(_loc2_,_loc2_ - this._lastUpdateTimeMs);
      ...
      private function simpleFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         priority = 0;
         while(priority < CoreComponentContext.const_106)
         {
            this.var_651[priority] = 0;
            receivers = this.var_270[priority];
            index = 0;
            length = receivers.length;
            while(index != length)
            {
      ...
      private function complexFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         while(priority < CoreComponentContext.const_106)
         {
            ...
            {
               if(this.var_651[priority] < priority)
               {
                  ++this.var_651[priority];
                  skip = true;
               }
            }
            if(!skip)
      ...
      public function setProfilerMode(param1:Boolean) : void
      {
         ...
         if(param1)
         {
            ...
            _loc2_ = 0;
            while(_loc2_ < CoreComponentContext.const_106)
            {
               _loc3_ = this.var_270[_loc2_];
