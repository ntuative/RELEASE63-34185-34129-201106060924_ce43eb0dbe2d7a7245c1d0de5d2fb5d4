package com.sulake.core.utils
{
   ...
   public class LibraryLoader extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1981:uint = 1;
      
      protected static const const_1980:uint = 2;
      
      protected static const const_1982:uint = 4;
      
      protected static const const_1983:uint = 3;
      
      protected static const name_4:uint = 7;
      
      public static const const_1684:int = 5;
      
      public static const LIBRARY_LOADER_FINALIZE:String = "LIBRARY_LOADER_FINALIZE";
      
      public static const const_1282:Boolean = false;
      
      public static const const_1736:int = 2;
      
      private static var var_1084:Array = new Array();
      
      private static var var_873:Array = new Array();
      ...
      protected var var_415:int = 0;
      
      protected var var_250:URLRequest;
      
      protected var _manifest:XML;
      
      protected var var_666:Class;
      
      protected var var_497:uint = 0;
      
      protected var _name:String;
      
      protected var var_1120:Boolean = false;
      
      protected var _debug:Boolean = false;
      
      protected var var_1121:Boolean = false;
      
      protected var var_2990:String = "";
      
      protected var var_2989:String = "";
      
      protected var var_2212:int;
      
      protected var var_1394:int;
      
      protected var var_1395:int;
      
      protected var var_3051:uint = 0;
      
      protected var var_2991:Boolean = true;
      ...
      protected static function queue(param1:LibraryLoader) : void
      {
         ...
         {
            param1.var_43.load(param1.var_250,param1._context);
         }
      }
      ...
      protected static function throttle() : void
      {
         ...
         if(const_1282)
         {
            ...
            while(_loc2_ > -1)
            {
               _loc1_ = var_873[_loc2_] as LibraryLoader;
            ...
            while(var_873.length < const_1736 && false)
            {
               _loc1_ = var_1084.shift() as LibraryLoader;
               if(!_loc1_.ready && !_loc1_.disposed)
               {
                  var_873.push(_loc1_);
                  _loc1_.var_43.load(_loc1_.var_250,_loc1_._context);
               }
            }
         }
      }
      ...
      {
         return !!this.var_250 ? this.var_250.url : null;
      }
      ...
      {
         return !!this.var_1120 ? uint(this.var_1394 - this.var_2212) : uint(getTimer() - this.var_2212);
      }
      ...
      override public function dispose() : void
      {
         if(!disposed)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DISPOSE,this.var_415,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
            try
            {
               this.var_43.close();
            }
      ...
      {
         this.var_250 = param1;
         this._name = parseNameFromUrl(this.var_250.url);
         this.var_1120 = false;
      ...
      {
         if(this.var_1121 && !_disposed)
         {
            this.var_1121 = false;
            if(!this.var_1120 && this.var_250)
            {
               this.var_1394 = -1;
      ...
      {
         if(!this.var_1120 && !_disposed && this.var_1395 > 0)
         {
            try
            {
               this.var_43.close();
            ...
            this.addRequestCounterToUrlRequest(this.var_250,const_1684 - this.var_1395);
            if(this.var_2991)
            {
               this.var_250.requestHeaders.push(new URLRequestHeader("pragma","no-cache"));
               this.var_250.requestHeaders.push(new URLRequestHeader("Cache-Control","no-cache"));
               this.var_2991 = false;
            }
            --this.var_1395;
            this.var_497 = 0;
            this.var_43.load(this.var_250,this._context);
            return true;
         }
         return false;
      }
      ...
      protected function loadEventHandler(param1:Event) : void
      {
         ...
         switch(param1.type)
         {
            ...
            case HTTPStatusEvent.HTTP_STATUS:
               this.var_415 = HTTPStatusEvent(param1).status;
               this.debug("Load event STATUS " + this.var_415 + " for file \"" + this.url + "\"");
               break;
            case Event.UNLOAD:
               this.debug("Load event UNLOAD for file \"" + this.url + "\"");
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_UNLOAD,this.var_415,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case ProgressEvent.PROGRESS:
               this.debug("Load event PROGRESS for file \"" + this.url + "\"  bytes: " + this.bytesLoaded + "/" + this.bytesTotal);
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_PROGRESS,this.var_415,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case IOErrorEvent.IO_ERROR:
               this.debug("Load event IO ERROR for file \"" + this.url + "\"");
               if(!this.handleHttpStatus(this.var_415))
               {
            ...
            case TimerEvent.TIMER:
               ...
               if(!_disposed)
               {
                  this.var_497 = LibraryLoader.name_4;
         ...
         if(this.var_497 == LibraryLoader.const_1983)
         {
         ...
         if(this.var_497 == LibraryLoader.name_4)
         {
            if(this.prepareLibrary())
            {
               ...
               this.removeEventListeners();
               throttle();
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE,this.var_415,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
      ...
      protected function prepareLibrary() : Boolean
      {
         ...
         if(this.var_666 == null)
         {
            if(!this.retry())
            {
               this.failure("Failed to find resource class \"" + this._name + "\" from library " + this.var_250.url + "!");
            }
            return false;
         }
      ...
      protected function debug(param1:String) : void
      {
         ...
         if(this._debug)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DEBUG,this.var_415,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         }
      }
      ...
      protected function failure(param1:String) : void
      {
         ...
         throttle();
         dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR,this.var_415,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         dispatchEvent(new Event(LIBRARY_LOADER_FINALIZE));
      }
