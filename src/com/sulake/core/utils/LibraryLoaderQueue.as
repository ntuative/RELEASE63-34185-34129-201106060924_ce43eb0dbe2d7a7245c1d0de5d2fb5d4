package com.sulake.core.utils
{
   ...
   public class LibraryLoaderQueue extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1736:int = 4;
      ...
      {
         return this.var_466.length + this.var_293.length;
      }
      ...
      private function next() : void
      {
         ...
         if(!disposed)
         {
            while(this.var_293.length < const_1736 && this.var_466.length > 0)
            {
               _loc1_ = this.var_466.shift();
               this.var_293.push(_loc1_);
               _loc1_.resume();
            }
         }
      }
      ...
      private function removeLoader(param1:LibraryLoader) : void
      {
         ...
         try
         {
            index = this.var_466.indexOf(loader);
            if(index > -1)
            {
               this.var_466.splice(index,1);
            }
            index = this.var_293.indexOf(loader);
            if(index > -1)
            {
               this.var_293.splice(index,1);
            }
         }
         catch(e:Error)
         {
      ...
      public function findLibraryLoaderByURL(param1:String, param2:Boolean = true) : LibraryLoader
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc3_ in this.var_293)
            {
               if(param2)
               {
                  if(_loc3_.url.indexOf(param1) == 0)
