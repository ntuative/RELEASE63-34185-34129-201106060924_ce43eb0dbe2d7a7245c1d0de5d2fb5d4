package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_1101:int = 3;
      
      private var var_487:AssetLoaderStruct;
      
      private var var_1102:Timer;
      ...
      private const const_2295:int = 100;
      
      private const const_1736:int = 2;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         if(_loc6_ == null)
         {
            this.var_487 = this._assets.loadAssetFromFile("figuremap",_loc5_,"text/xml");
         ...
         this.var_1102 = new Timer(this.const_2295,1);
         this.var_1102.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_487)
         {
            this.var_487.addEventListener(AssetLoaderEvent.const_29,this.onConfigurationComplete);
            this.var_487.addEventListener(AssetLoaderEvent.const_43,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_487)
         {
            this.var_487.removeEventListener(AssetLoaderEvent.const_29,this.onConfigurationComplete);
            this.var_487.removeEventListener(AssetLoaderEvent.const_43,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         if(this.var_1101 <= 0)
         {
            HabboWebTools.logEventLog("Figuremap download error " + this.var_1368);
            Logger.log("AVATAR ASSET ERROR: Could not download figuremap.xml.");
         }
         else
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_1101);
            if(this.var_1368.indexOf("?") > 0)
            {
               _loc2_ = this.var_1368 + "&retry=" + this.var_1101;
            }
            else
            {
               _loc2_ = this.var_1368 + "?retry=" + this.var_1101;
            ...
            this.var_487 = this._assets.loadAssetFromFile("figuremap",_loc3_,"text/xml");
            this.addMapLoaderEventListeners();
         }
      }
      ...
      private function loadFigureMapData(param1:XML) : void
      {
         ...
         this.var_2141 = true;
         for each(_loc2_ in this.var_1369)
         {
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2963,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1367[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(_loc3_);
               this.var_1367[_loc5_] = _loc6_;
            }
         }
      }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_2141)
         {
            this.var_1369.push([param1,param2]);
            return;
         }
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = this.var_557[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               this.var_557[_loc3_] = _loc6_;
            }
            this.var_1103[_loc3_] = _loc4_;
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         for(_loc3_ in this.var_1103)
         {
            ...
            if(_loc7_)
            {
               ...
               delete this.var_557[_loc3_];
            }
         }
         for each(_loc3_ in _loc2_)
         {
            delete this.var_1103[_loc3_];
         }
         while(_loc6_ < this.var_888.length)
         {
            _loc5_ = this.var_888[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_888.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_1102.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_887.length > 0 && this.var_888.length < this.const_1736)
         {
            _loc1_ = this.var_887[0];
            _loc1_.startDownloading();
            this.var_888.push(this.var_887.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_887.indexOf(param1) == -1 && this.var_888.indexOf(param1) == -1)
         {
