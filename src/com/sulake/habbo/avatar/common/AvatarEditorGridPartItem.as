package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_73:IAvatarEditorCategoryModel;
      
      private var _window:IWindowContainer;
      
      private var var_1487:IWindow;
      ...
      private var var_641:Rectangle;
      
      private var var_1836:int = 0;
      
      private var var_448:IAvatarRenderManager;
      
      private var var_1113:Boolean;
      
      private var var_1837:BitmapData;
      
      private const const_2267:int = 13421772;
      
      private const const_2266:int = 16777215;
      
      private const const_373:Array = [2,6,0,4,3,1];
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         this.var_779 = param3;
         this._window = param1;
         this.var_1487 = this._window.findChildByTag("BG_COLOR");
         this._colors = param4;
         this.var_1838 = param5;
         if(param3 == null)
         {
            this.var_722 = new BitmapData(1,1,true,16777215);
         }
         if(param3 != null)
         {
            _loc7_ = param3.parts;
            ...
            {
               this.var_1836 = Math.max(this.var_1836,_loc6_.colorLayerIndex);
            }
         }
         this.var_448 = this.var_73.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_722 != null && !this.var_1838)
            {
               _loc3_ = this.var_722;
            }
            else
         ...
         if(this.isSelected)
         {
            this.var_1487.color = this.const_2267;
         }
         else
         {
            this.var_1487.color = this.const_2266;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_448.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_448.isFigureReady(_loc1_))
         {
            this.var_448.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_373[_loc6_] + "_" + FigureData.const_154;
               _loc4_ = this.var_448.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
               while(!_loc7_ && _loc6_ < this.const_373.length)
               {
                  _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_373[_loc6_] + "_" + FigureData.const_154;
                  _loc4_ = this.var_448.getAssetByName(_loc3_) as BitmapDataAsset;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_641 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1837)
               {
                  _loc9_ = this.var_73.controller.assets.getAssetByName("avatar_editor_download_icon");
                  this.var_1837 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_641.width,this.var_641.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_641.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_641.y;
               if(this.var_1838 && _loc2_.colorLayerIndex > 0)
               {
