package com.sulake.habbo.catalog.purchase
{
   ...
   public class PurchaseConfirmationDialog implements IGetImageListener, IDisposable
   {
      ...
      private const const_2278:int = 10;
      ...
      private const const_2005:uint = 4.293848814E9;
      
      private const const_2006:uint = 4.294967295E9;
      
      private const const_1463:uint = 4.291613146E9;
      ...
      public function showOffer(param1:IHabboCatalog, param2:IRoomEngine, param3:IPurchasableOffer, param4:int, param5:String = "", param6:Array = null) : void
      {
         ...
         this.var_1290 = param3.offerId;
         this.var_1586 = param4;
         this.var_1585 = param5;
         this.var_232 = param6;
      ...
      public function onIsOfferGiftable(param1:IsOfferGiftableMessageParser) : void
      {
         ...
         if(this.var_1290 != param1.offerId)
         {
            return;
         }
      ...
      private function showGiftDialog() : void
      {
         ...
         if(_loc6_ != null)
         {
            _loc6_.addEventListener(WindowEvent.const_109,this.onNameInputChange);
            _loc6_.addEventListener(WindowMouseEvent.const_44,this.onNameInputMouseDown);
            _loc6_.addEventListener(WindowKeyboardEvent.const_365,this.onNameInputKeyUp);
         }
         ...
         if(_loc7_ != null)
         {
            ...
            _loc7_.addEventListener(WindowEvent.const_329,this.onMessageInputFocus);
         }
         this.var_1980 = true;
         this.var_1981 = true;
         if(_loc1_.isEnabled)
         {
            ...
            if(_loc12_ != null)
            {
               ...
               _loc12_.addEventListener(WindowEvent.const_496,this.onFreeWrappingUnSelect);
            }
            ...
            this.var_1817 = _loc1_.stuffTypes;
            this.var_840 = _loc1_.boxTypes;
            this.var_838 = _loc1_.ribbonTypes;
            this.var_1291 = this.var_1817[0];
            this.var_397 = this.var_840[0];
            this.var_398 = this.var_838[0];
            this.initColorGrid();
      ...
      private function updatePreview() : void
      {
         if(this.var_398 < 0)
         {
            this.var_398 = this.var_838.length - 1;
         }
         if(this.var_398 > this.var_838.length - 1)
         {
            this.var_398 = 0;
         }
         if(this.var_397 < 0)
         {
            this.var_397 = this.var_840.length - 1;
         }
         if(this.var_397 > this.var_840.length - 1)
         {
            this.var_397 = 0;
         }
         var _loc1_:int = this.var_840[this.var_397] * 1000 + this.var_838[this.var_398];
         if(this._window == null)
         {
            return;
         }
         ...
         var _loc2_:ImageResult = this._roomEngine.getFurnitureImage(this.var_1291,new Vector3d(180),64,this,0,_loc1_.toString());
         if(_loc2_ == null)
         {
            return;
         }
         this.var_1584 = _loc2_.id;
      ...
      private function giveGift() : void
      {
         ...
         var _loc7_:int = !!_loc6_ ? 0 : int(this.var_1291);
         var _loc8_:int = !!_loc6_ ? 0 : int(this.var_840[this.var_397]);
         var _loc9_:int = !!_loc6_ ? 0 : int(this.var_838[this.var_398]);
         this._catalog.purchaseProductAsGift(this.var_1586,this.var_1290,this.var_1585,_loc2_,_loc4_,_loc7_,_loc8_,_loc9_);
      }
      
      private function onBuyButtonClick(param1:WindowEvent) : void
      {
         this._catalog.purchaseProduct(this.var_1586,this.var_1290,this.var_1585);
         this.dispose();
      }
      ...
      private function onNameInputChange(param1:WindowEvent) : void
      {
         ...
         if(this.var_2732 == _loc2_.text)
         {
            return;
         }
         var _loc3_:String = _loc2_.text.toLowerCase();
         var _loc4_:* = [];
         for each(_loc5_ in this.var_232)
         {
            if(_loc5_.toLowerCase().search(_loc3_) != -1)
            {
               _loc4_.push(_loc5_);
            }
            if(_loc4_.length >= this.const_2278)
            {
               break;
            }
         }
         this.updateSuggestions(_loc4_);
         this.var_2732 = _loc2_.text;
      }
      ...
      private function onNameInputKeyUp(param1:WindowEvent) : void
      {
         ...
         {
            case Keyboard.UP:
               this.highlightSuggestion(this.var_399 - 1);
               break;
            case Keyboard.DOWN:
               this.highlightSuggestion(this.var_399 + 1);
               break;
      ...
      {
         if(this.var_282 == null || !this.var_282.visible)
         {
            return;
         }
         var _loc1_:IItemListWindow = this.var_282.findChildByName("suggestion_list") as IItemListWindow;
         if(_loc1_ == null)
         {
            return;
         }
         var _loc2_:IWindowContainer = _loc1_.getListItemAt(this.var_399) as IWindowContainer;
         if(_loc2_ == null)
         {
            return;
         }
      ...
      private function updateSuggestions(param1:Array) : void
      {
         ...
         if(this.var_282 == null)
         {
            this.var_282 = this._window.findChildByName("suggestion_container") as IWindowContainer;
         }
         if(this.var_839 == null)
         {
            this.var_839 = this.createWindow("suggestion_list_item") as IWindowContainer;
         }
         if(this.var_282 == null || this.var_839 == null)
         {
            return;
         }
         var _loc2_:IItemListWindow = this.var_282.findChildByName("suggestion_list") as IItemListWindow;
         if(_loc2_ == null)
         {
            return;
         }
         ...
         {
            _loc5_ = this.var_839.clone() as IWindowContainer;
            if(_loc5_ != null)
            {
      ...
      private function onSuggestionsMouseOver(param1:WindowEvent) : void
      {
         ...
         var _loc3_:IItemListWindow = this.var_282.findChildByName("suggestion_list") as IItemListWindow;
         if(_loc3_ == null)
         {
            return;
         }
      ...
      private function highlightSuggestion(param1:int) : void
      {
         ...
         _loc3_ = _loc2_.getListItemAt(this.var_399) as IWindowContainer;
         if(_loc3_ != null)
         {
            _loc3_.color = this.getColor(this.var_399);
         }
         this.var_399 = param1;
         if(this.var_399 < 0)
         {
            this.var_399 = _loc2_.numListItems - 1;
         }
         if(this.var_399 >= _loc2_.numListItems)
         {
            this.var_399 = 0;
         }
         _loc3_ = _loc2_.getListItemAt(this.var_399) as IWindowContainer;
         if(_loc3_ != null)
         {
            _loc3_.color = this.const_1463;
         }
      }
      
      private function getColor(param1:int) : uint
      {
         return param1 % 2 == 0 ? uint(this.const_2005) : uint(this.const_2006);
      }
      ...
      {
         this.var_1291 = param1.target.id;
         this.updateColorGrid();
         this.updatePreview();
      }
      ...
      private function updateColorGrid() : void
      {
         ...
         while(_loc4_ < _loc1_.numGridItems)
         {
            ...
            if(_loc2_ != null)
            {
               ...
               if(_loc3_ != null)
               {
                  _loc3_.visible = _loc2_.id == this.var_1291;
               }
            }
            _loc4_++;
         }
