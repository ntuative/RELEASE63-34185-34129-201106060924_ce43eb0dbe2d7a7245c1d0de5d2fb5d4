package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class SpacesNewCatalogWidget extends ItemGridCatalogWidget implements IDisposable, ICatalogWidget, IItemGrid
   {
      ...
      private var var_453:ISelectorWindow;
      ...
      public function SpacesNewCatalogWidget(param1:IWindowContainer, param2:ISessionDataManager)
      {
         ...
         this.var_673 = [0,0,0];
         this.var_110 = ["group.walls","group.floors","group.views"];
         super(param1,param2);
         this.var_453 = _window.findChildByName("groups") as ISelectorWindow;
         if(this.var_453)
         {
            _loc3_ = 0;
            while(_loc3_ < this.var_453.numSelectables)
            {
               _loc4_ = this.var_453.getSelectableAt(_loc3_);
               if(_loc4_ is ISelectableWindow)
               {
                  _loc4_.addEventListener(WindowEvent.const_50,this.onSelectGroup);
               }
               _loc3_++;
            }
         }
      ...
      override public function init() : Boolean
      {
         ...
         this.switchCategory(this.var_110[this.var_543]);
      ...
      {
         var _loc2_:int = this.var_673[this.var_543];
         var _loc3_:Offer = this.var_212[this.var_543][_loc2_];
      ...
      {
         var _loc2_:* = null;
         if(param1 > -1 && param1 < var_495.numGridItems)
         {
            _loc2_ = this.var_212[this.var_543][param1];
      ...
      override public function select(param1:IGridItem) : void
      {
         ...
         this.var_673[this.var_543] = (this.var_212[this.var_543] as Array).indexOf(_loc2_);
         this.updateRoomPreview();
      }
      
      private function updateRoomPreview() : void
      {
         var _loc1_:int = this.var_673[0];
         var _loc2_:int = this.var_673[1];
         var _loc3_:int = this.var_673[2];
         var _loc4_:Offer = this.var_212[0][_loc1_];
         var _loc5_:Offer = this.var_212[1][_loc2_];
         var _loc6_:Offer = this.var_212[2][_loc3_];
      ...
      private function createOfferGroups() : Boolean
      {
         ...
         {
            if(!(_loc1_.pricingModel == Offer.const_416 || _loc1_.pricingModel == Offer.const_452))
            {
               continue;
            }
            ...
            while(this.var_212.length < this._groupNames.length)
            {
               this.var_212.push(new Array());
            }
            switch(_loc4_)
            {
               case "floor":
                  (this.var_212[_loc5_] as Array).push(_loc1_);
                  break;
               case "wallpaper":
                  (this.var_212[_loc5_] as Array).push(_loc1_);
                  break;
      ...
      private function onSelectGroup(param1:WindowEvent) : void
      {
         ...
         if(_loc2_)
         {
            _loc3_ = this.var_453.getSelectableIndex(_loc2_);
      ...
      private function switchCategory(param1:String) : void
      {
         ...
         this.var_453.setSelected(this.var_453.getSelectableByName(param1));
         var _loc2_:int = -1;
         switch(param1)
         {
            case "group.walls":
         ...
         if(_loc2_ > -1)
         {
            ...
            if(var_495)
            {
               var_495.destroyGridItems();
            }
            page.replaceOffers(this.var_212[this.var_543],false);
            resetTimer();
            populateItemGrid();
            ...
            _loc3_ = this.var_673[this.var_543];
