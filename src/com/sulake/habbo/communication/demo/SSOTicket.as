package com.sulake.habbo.communication.demo
{
   ...
   public class SSOTicket extends EventDispatcher
   {
      ...
      public static const const_361:String = "failure";
      ...
      private var var_1579:IHabboWebLogin;
      ...
      public function SSOTicket(param1:IAssetLibrary, param2:IHabboWebLogin, param3:String, param4:Boolean = true)
      {
         ...
         this.var_1578 = "http://" + param3 + "/client";
         if(!param4)
         {
            this.var_1579.init();
            this.var_1579.addEventListener(HabboWeb.const_240,this.requestClientURL);
         }
         else
         {
      ...
      {
         var _loc2_:String = this.var_1578;
         if(this._assets.hasAsset(_loc2_))
         {
            Logger.log("[CoreLocalizationManager] reload localization for url: " + this.var_1578);
         }
         var _loc3_:URLRequest = new URLRequest(this.var_1578);
         if(this._assets.hasAsset(_loc2_))
         {
      ...
      private function clientURLComplete(param1:Event = null) : void
      {
         ...
         {
            this.var_1579.requestReAuthenticate();
         }
         else
         {
            try
            {
               this.var_1969 = /\"sso.ticket\" : \"(.*?)\"/.exec(data)[1];
               this.var_1911 = /\"flash.client.url\" : \"(.*?)\"/.exec(data)[1];
               facebookData = /\"facebook.user\" : \"(.*?)\"/.exec(data);
               if(facebookData)
               {
                  this.var_2714 = Boolean(facebookData[1]);
               }
               if(this.var_1969.length > 0)
               {
                  dispatchEvent(new Event(const_415));
               }
               else
               {
                  dispatchEvent(new Event(const_361));
               }
            }
            catch(e:Error)
            {
