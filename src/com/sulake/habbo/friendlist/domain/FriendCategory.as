package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategory implements IDisposable
   {
      ...
      {
         if(this.var_965 >= this.getPageCount())
         {
            this.var_965 = Math.max(0,this.getPageCount() - 1);
         }
      }
      
      public function getPageCount() : int
      {
         return Math.ceil(this.var_232.length / const_409);
      }
      
      public function getStartFriendIndex() : int
      {
         this.checkPageIndex();
         return this.var_965 * const_409;
      }
      
      public function getEndFriendIndex() : int
      {
         this.checkPageIndex();
         return Math.min((this.var_965 + 1) * const_409,this.var_232.length);
      }
