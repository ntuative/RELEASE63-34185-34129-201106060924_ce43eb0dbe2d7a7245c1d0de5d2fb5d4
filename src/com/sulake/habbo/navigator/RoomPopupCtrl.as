package com.sulake.habbo.navigator
{
   ...
   public class RoomPopupCtrl extends PopupCtrl
   {
      ...
      private var var_130:GuestRoomData;
      
      private var var_2743:RoomThumbnailRenderer;
      ...
      public function RoomPopupCtrl(param1:HabboNavigator, param2:int, param3:int)
      {
         ...
         this.var_2743 = new RoomThumbnailRenderer(param1);
         this.var_1024 = new TagRenderer(param1,hideInstantly);
      }
      ...
      override public function refreshContent(param1:IWindowContainer) : void
      {
         ...
         {
            this.var_193 = IWindowContainer(param1.findChildByName("details_container"));
         }
         this.var_193.visible = true;
         Util.hideChildren(this.var_193);
         this.refreshRoomName(this.var_193,this.var_130);
         if(this.var_130.ownerName != "" && this.var_130.ownerName != "-")
         {
         }
         this.refreshOwnerName();
         this.refreshTextWithCaption("roomctg",this.var_193,!!this.var_130.event ? this.getEventCtg(this.var_130.categoryId) : this.getRoomCtg(this.var_130.categoryId));
         this.refreshRoomDesc(this.var_193,this.var_130);
         this.refreshExtraCont();
         this.refreshInfo(this.var_193,"trading_allowed",this.var_130.allowTrading);
         this.refreshInfo(this.var_193,"doormode_doorbell",this.var_130.doorMode == 1);
         this.refreshInfo(this.var_193,"doormode_password",this.var_130.doorMode == 2);
         Util.moveChildrenToColumn(this.var_193,["roomname","roomctg_cont","roomowner_cont","roomdesc","extra_cont","doormode_doorbell","doormode_password","trading_allowed"],0,0);
         this.var_193.height = Util.getLowestPoint(this.var_193);
      }
      
      private function refreshOwnerName() : void
      {
         this.refreshTextWithCaption("roomowner",this.var_193,this.var_130.ownerName);
         var _loc1_:ITextWindow = ITextWindow(this.var_193.findChildByName("roomowner"));
         if(this.var_130.ownerName != "" && this.var_130.ownerName != "-")
         {
            _loc1_.procedure = this.onOwnerName;
            _loc1_.setParamFlag(HabboWindowParam.const_47,true);
            _loc1_.underline = true;
         }
         else
         {
            _loc1_.procedure = null;
            _loc1_.setParamFlag(HabboWindowParam.const_47,false);
      ...
      {
         var _loc1_:IWindowContainer = IWindowContainer(this.var_193.findChildByName("extra_cont"));
         Util.hideChildren(_loc1_);
         this.var_1024.refreshTags(_loc1_,this.var_130.tags);
         this.refreshTextWithCaption("rating",_loc1_,"" + this.var_130.score);
         if(this.var_130.event)
         {
            this.refreshTextWithCaption("startedat",_loc1_,"" + this.var_130.eventCreationTime);
         }
         this.var_2743.refreshThumbnail(_loc1_,this.var_130.thumbnail,false);
      ...
      private function onOwnerName(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(_loc3_ != null)
         {
            navigator.mainViewCtrl.startSearch(Tabs.const_200,Tabs.const_267,_loc3_.text);
            this.hideInstantly();
         }
      }
   }
