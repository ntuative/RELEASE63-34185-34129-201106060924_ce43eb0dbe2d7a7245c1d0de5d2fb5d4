package com.sulake.habbo.roomevents.userdefinedroomevents.help
{
   ...
   public class UserDefinedRoomEventsHelp
   {
      ...
      private function prepareWindow() : void
      {
         ...
         this.var_1695 = IWindowContainer(this.find(this._window,"help_container"));
         this.var_1134 = this.var_1695.findChildByName("button_previous") as IContainerButtonWindow;
         if(this.var_1134)
         {
            this.var_1134.procedure = this.onButtonClick;
         }
         this.var_1135 = this.var_1695.findChildByName("button_next") as IContainerButtonWindow;
         if(this.var_1135)
         {
            this.var_1135.procedure = this.onButtonClick;
         }
      ...
      {
         if(this.var_251 >= this.var_67.length - 1)
         {
            this.var_1135.disable();
         }
         else
         {
            this.var_1135.enable();
         }
         if(this.var_251 <= 0)
         {
            this.var_1134.disable();
         }
         else
         {
            this.var_1134.enable();
         }
      }
      ...
      {
         ++this.var_251;
         this.var_251 = Math.min(this.var_251,this.var_67.length - 1);
         this.openPage();
      }
      ...
      private function initLocalizations() : void
      {
         ...
         while(this.var_88.localization.getLocalization("wiredfurni.help." + _loc1_ + ".title"))
         {
            ...
            this.var_67.push([_loc2_,_loc3_,_loc4_]);
            _loc1_++;
         }
      }
      ...
      private function localize() : void
      {
         ...
         if(!this.var_67 || this.var_67.length == 0)
         {
            return;
         }
         if(_loc1_)
         {
            _loc1_.text = ILocalization(this.var_67[this.var_251][0]).value;
         }
         if(_loc2_)
         {
            _loc2_.text = ILocalization(this.var_67[this.var_251][2]).value;
         }
         if(_loc3_)
         {
            _loc4_ = ILocalization(this.var_67[this.var_251][1]).value;
