package com.sulake.habbo.sound.trax
{
   ...
   public class TraxSequencer implements IHabboSound, IDisposable
   {
      ...
      private static const const_1461:uint = 88000;
      
      private static const const_1462:uint = 88000;
      
      private static const const_306:Vector.<int> = new Vector.<int>(const_197,true);
      ...
      {
         return this.var_442 / const_118;
      }
      
      public function set fadeOutSeconds(param1:Number) : void
      {
         this.var_442 = int(param1 * const_118);
      }
      
      public function get fadeInSeconds() : Number
      {
         return this.var_764 / const_118;
      }
      
      public function set fadeInSeconds(param1:Number) : void
      {
         this.var_764 = int(param1 * const_118);
      }
      ...
      private function prepareSequence() : Boolean
      {
         ...
         var _loc2_:int = 0;
         while(_loc2_ < this.var_1187.channels.length)
         {
            _loc3_ = new Map();
            _loc4_ = this.var_1187.channels[_loc2_] as TraxChannel;
            _loc5_ = 0;
            _loc6_ = 0;
            _loc7_ = 0;
            while(_loc7_ < _loc4_.itemCount)
            {
               ...
               _loc9_ = this.var_1793.getValue(_loc8_) as TraxSample;
               _loc9_.setUsageFromSong(this.var_1710,_loc1_);
               if(_loc9_ == null)
               {
      ...
      public function play(param1:Number = 0.0) : Boolean
      {
         ...
         this.var_1473 = param1 * const_118;
         this.var_1188 = 0;
         this.var_1794 = 0;
         this.var_441 = this.var_949.play();
      ...
      {
         if(this.var_442 > 0 && !this.var_623)
         {
      ...
      private function stopWithFadeout() : void
      {
         if(this.var_624 == null)
         {
            this.var_763 = true;
            this.var_516 = 0;
            this.var_624 = new Timer(this.var_1474 + this.var_442 / (const_118 / 1000),1);
            this.var_624.start();
      ...
      {
         return Math.ceil(param1 / const_1461);
      }
      ...
      private function mixChannelsIntoBuffer() : void
      {
         ...
         var _loc2_:int = this.var_439.length;
         var _loc3_:* = null;
         var _loc4_:int = _loc2_ - 1;
         while(_loc4_ >= 0)
         {
            _loc5_ = this.var_439[_loc4_];
            ...
            _loc8_ = const_197;
            if(this.var_366 - this.var_206 < _loc8_)
            {
               _loc8_ = this.var_366 - this.var_206;
            }
            _loc9_ = 0;
            while(_loc9_ < _loc8_)
            {
               ...
               if(_loc4_ == _loc2_ - 1)
               {
                  if(_loc3_ != null)
                  {
                     _loc3_.setSample(const_306,_loc9_,_loc12_);
                     _loc9_ += _loc12_;
                  }
                  else
                  {
               ...
               else
               {
                  if(_loc3_ != null)
                  {
                     _loc3_.addSample(const_306,_loc9_,_loc12_);
                  }
                  _loc9_ += _loc12_;
               }
               if(_loc9_ < _loc8_)
      ...
      {
         var _loc1_:int = this.var_366 < this.var_1473 ? int(this.var_366) : (this.var_1473 > 0 ? int(this.var_1473) : int(this.var_366));
         if(this.var_206 > _loc1_ + this.var_1474 * (const_118 / 1000) && !this.var_623)
         {
            this.var_623 = true;
            if(this.var_440 != null)
            {
               this.var_440.reset();
               this.var_440.removeEventListener(TimerEvent.TIMER_COMPLETE,this.onPlayingComplete);
            }
            this.var_440 = new Timer(2,1);
            this.var_440.start();
            this.var_440.addEventListener(TimerEvent.TIMER_COMPLETE,this.onPlayingComplete);
         }
         else if(this.var_206 > _loc1_ - this.var_442 && !this.var_763)
         {
            this.var_951 = false;
            this.var_763 = true;
            this.var_516 = 0;
      ...
      {
         if(param1.position > this.var_1188)
         {
            ++this.var_1794;
            Logger.log("Audio buffer under run...");
            this.var_1188 = param1.position;
         }
         if(this.volume > 0)
         {
         ...
         if(this.var_366 - this.var_206 < _loc2_)
         {
            _loc2_ = this.var_366 - this.var_206;
            if(_loc2_ < 0)
            {
               _loc2_ = 0;
            }
         ...
         this.var_206 += const_197;
         this.var_1188 += const_197;
         if(this.var_441 != null)
         {
            this.var_1474 = param1.position / const_118 * 1000 - this.var_441.position;
         }
      ...
      private function writeAudioToOutputStream(param1:ByteArray, param2:int) : void
      {
         ...
         if(param2 > 0)
         {
            ...
            else
            {
               if(this.var_951)
               {
                  _loc5_ = 1 / this.var_764;
                  _loc6_ = this.var_950 / Number(this.var_764);
                  this.var_950 += const_197;
                  if(this.var_950 > this.var_764)
                  {
                     this.var_951 = false;
                  }
               }
               else if(this.var_763)
               {
                  _loc5_ = -1 / this.var_442;
                  _loc6_ = 1 - this.var_516 / Number(this.var_442);
                  this.var_516 += const_197;
                  if(this.var_516 > this.var_442)
                  {
                     this.var_516 = this.var_442;
                  }
               }
      ...
      private function writeMixingBufferToOutputStream(param1:ByteArray, param2:int) : void
      {
         ...
         while(_loc4_ < param2)
         {
            _loc3_ = Number(Number(const_306[_loc4_]) * 0);
      ...
      private function writeMixingBufferToOutputStreamWithFade(param1:ByteArray, param2:int, param3:Number, param4:Number) : void
      {
         ...
         while(_loc6_ < param2)
         {
            ...
            _loc5_ = Number(Number(const_306[_loc6_]) * 0 * param3);
            param3 += param4;
         ...
         else if(param3 > 1)
         {
            while(_loc6_ < param2)
            {
               _loc5_ = Number(Number(const_306[_loc6_]) * 0);
               param3 += param4;
               param1.writeFloat(_loc5_);
               param1.writeFloat(_loc5_);
               _loc6_++;
      ...
      {
         this.stopImmediately();
         this._events.dispatchEvent(new SoundCompleteEvent(SoundCompleteEvent.TRAX_SONG_COMPLETE,this.var_1710));
      }
